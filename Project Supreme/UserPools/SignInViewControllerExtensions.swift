...
extension SignInViewController: AWSCognitoIdentityPasswordAuthentication {
    ...
    func didCompleteStepWithError(_ error: Error?) {
        // checking for an error during the sign in process
        if let error = error as? NSError {
            if error.code == awsErrorType.unconfirmedUser { // unconfirmed user attempting to sign in
                
                // send them confirmation code and take them to the SignUp Confirmation Screen
                let sendSMS = UIAlertAction(title: "Send SMS", style: .default, handler: { (action: UIAlertAction) in
                    ...
                    userPoolSignUpConfirm.onSendConfirmationCode(self)
                    
                    self.supremePresent(viewController: userPoolSignUpConfirm, animated: true)
                })
                
                let ac = UIAlertController(title: "Hold up", message: "You need to confirm your account before you can use it!", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Cancel", style: .cancel))
                ac.addAction(sendSMS)
                self.supremePresent(viewController: ac, animated: true)
                
                return
            }
            
            self.supremeShowError(title: String(describing: error.userInfo["__type"]!), message: String(describing: error.userInfo["message"]!), action: nil)
        }
    }
}
    ...
    func handleUserPoolSignInFlowStart() {
        guard let username = self.usernameTextField.text, !username.isEmpty,
            let password = self.passwordTextField.text, !password.isEmpty else {
                ...
                self.supremeShowError(title: "Missing Username / Password", message: "Please enter a valid username / password.", action: nil)
                return
        }
        ...
        self.passwordAuthenticationCompletion?.set(result: AWSCognitoIdentityPasswordAuthenticationDetails(username: username.lowercased(), password: password)) // set username to lowercased - all users in userpool are lowercased
    }
}
